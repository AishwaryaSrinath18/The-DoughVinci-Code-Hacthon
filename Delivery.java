    ...
    public List<Rider> optimizeDelivery() {
        ...
        for (Order order : orders) {
            ...
            for (Rider rider : riders) {
                // Check if the rider already has 2 orders
                if (rider.orders.size() >= 2) {
                    continue; // Skip this rider if they already have 2 orders
                }
                ...
                // Rule #1
                if (order.kitchenId.equals(lastOrder.kitchenId) && //from same kitchen
                        order.customerId.equals(lastOrder.customerId) && //to same customer
                        Math.abs(order.pickupTime - lastOrder.pickupTime) <= 10) { //10 mins apart ready
                    rider.addOrder(order);//same rider is assigned
                    assigned = true;
                    break;
                }

                // Rule #2
                else if (!order.kitchenId.equals(lastOrder.kitchenId) && //different kitchen
                        order.customerId.equals(lastOrder.customerId) && //same customer
                        Math.abs(order.pickupTime - lastOrder.pickupTime) <= 10 && //10 mins apart
                        cityGraph.calculateDistance(order.kitchenId, lastOrder.kitchenId) <= 1) //kitchens are 1km apart
                {
                    rider.addOrder(order);
                    assigned = true;
                    break;
                ...
                // Rule #3
                else if (order.kitchenId.equals(lastOrder.kitchenId) && //same kitchen
                        !order.customerId.equals(lastOrder.customerId) && //different customers
                        Math.abs(order.pickupTime - lastOrder.pickupTime) <= 10 && //10 mins apart
                        cityGraph.calculateDistance(order.customerId, lastOrder.customerId) <= 1) //1km apart
                {
                    rider.addOrder(order);
                    assigned = true;
                    break;
